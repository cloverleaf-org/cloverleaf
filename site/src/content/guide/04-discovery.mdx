...
The walker's RFC auto-advance treats these RFC-direct tasks as first-class. After every Plan completes, the walker asks `cloverleaf-cli rfc-tasks <RFC-ID>` whether the RFC can advance — and that check considers both sibling Plans and standalone tasks of the same RFC. An in-flight standalone task blocks the advance; a merged one counts toward "at least one delivered."
