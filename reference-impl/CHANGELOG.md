...
- `lib/council.ts` — `council-plan` now resolves a per-member `substitutions` map alongside `promptPath`. The council runner substituted a fixed five tokens (`{{task}}`, `{{branch}}`, `{{base_branch}}`, `{{repo_root}}`, `{{diff}}`), a set complete only for `security-reviewer.md`; `reviewer.md`'s `{{test_rules}}`, `qa.md`'s `{{qa_rules}}` and `ui-reviewer.md`'s `{{affected_routes}}` / `{{ui_review_config}}` arrived unresolved on the council path — the only automatic one — silently disabling the 0.10.0 test-runner agnosticism and leaving members to improvise a test command while still returning a confident `pass`. `test_rules`/`qa_rules` carry the qa-rules **document** (`{ rules: [...] }`, consumer override then packaged default), the shape those prompts document, not `loadQaRulesConfig()`'s bare array. Resolution is side-effect free: `{{preview_port}}` has no configured default and no non-allocating resolver, so it is omitted rather than fabricated and stays the dispatcher's job. New `loadQaRulesDocument()` in `lib/qa-rules.ts`; `loadQaRulesConfig()` is unchanged for callers that execute rules rather than prompt with them.
- `lib/council.ts` — the council result artifact's `security.basis` now distinguishes a security member that was never configured from one that was configured but never reached. `basis` was derived from `council.members` — the members that actually *ran* — so a profile with no security member and a blocking security member the council stopped before reaching both rendered as `no security member configured; advanced under council authority`. The artifact is the durable record a human reads when asking whether security was consulted, and for the second case that record was false. `applyCouncilVerdict` now classifies the absence against the gate's resolved plan (`resolveCouncilPlan`, consulted only when there is no security verdict to explain, and before any transition mutates the task the gate binding selects a profile from). A cause is named only when the run satisfies one of the council's two stop rules — an `escalate` short-circuit, or a **blocking** bounce under `on_round_bounce: stop` — so a planned member absent for any other reason (a non-blocking bounce, a member the runner never dispatched) is reported as unexplained rather than given an invented cause, and a `council.json` that will not resolve says so instead of failing the apply. A `when`-excluded member is not distinguished from an unconfigured one — the resolved plan filters `when`-inactive members out, and both are truthfully "no security member for this task". **Gating behavior is unchanged**: the `security_review_verdict` backstop and `gating_verdict_set` (the v0.8.1 guarantee) are byte-for-byte untouched; only the audit string changed.
