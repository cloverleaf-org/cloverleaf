...
### Added
- `tests/package-contract.test.ts` gains a third roster, guarding both plugin manifests against their `package.json`. Unlike the lockfile and `VERSION` guards, which were green on arrival and provable only by mutation, this one was **red on arrival** against a real pre-existing failure — the first of the three to be driven by an actual defect rather than a planted one. The two files are guarded against `package.json` rather than against each other, because `claude plugin tag` already validates that they agree with one another while nothing validated either against `package.json`, which is the axis that actually drifted. They are anchored differently because they sit differently: a `plugin.json` describes the package whose directory encloses `.claude-plugin`, so its anchor is the parent's `package.json`, whereas a marketplace entry names its plugin through `source` and the repo root has no `package.json` at all — so each entry resolves through that `source`, which also means a second plugin added to the catalogue is guarded the day it arrives rather than the day someone extends a list. A `source` is relative to the marketplace root and not to the `.claude-plugin` directory holding the catalogue; resolving it the other way reads as `ENOENT` rather than as a version failure, so the distinction is pinned in a comment. Three roster pins rather than two — the plugin manifests, the marketplace files, and the entries *within* a catalogue — since an entry silently dropped from `plugins[]` would generate no assertions and read as a pass. Not part of the published package.

### Changed
- ⚠ **Correction to the `0.13.5` entry below, and to the `0.13.5` release notes.** Both stated that these two manifests were deliberate "decoys" that "bump on feature releases only", left behind by the patch train on purpose. That was asserted from an inherited note rather than measured, and it is wrong on both counts. The version field is functional, not decorative, as above. And the release history records no such rule: measured across all 46 `reference-impl-v*` tags there are five completed lag episodes, of which **two closed on a patch release** (`0.6.5`, `0.7.4`), while the **`0.7.0` minor passed straight through a lag without closing it**. Patch trains have both bumped these files and skipped them. The `9.9.9` mutations cited in that entry remain accurate as a description of *behaviour* — the guard genuinely did not fire on those files at the time — but the stated reason for excluding them was not a rule, only an unexamined habit.

## 0.13.5 — 2026-09-02

**Patch: the version a release claims is now checked everywhere it is written, not just where it is read.**

### Added
- `tests/package-contract.test.ts` gains a lockfile-version guard. A package's version has two twins in its `package-lock.json` — the lockfile's root `.version` and `.packages[""].version` — and nothing checked either one. The gap is not hypothetical: `standard/package-lock.json` sat at `0.8.1` through the whole of the 0.8.2 release while `standard/package.json` said `0.8.2`, and was found by hand part-way through cutting 0.8.3. Nothing downstream complains, because npm never publishes a lockfile, so the drift is visible only from inside this repo — which is how a release carries it end to end. `reference-impl`'s own two fields stayed in sync only because a release note asks a human to remember them; this makes two of that six-site bump list self-enforcing from here on. The roster is derived by walking the tree for lockfiles rather than naming the three packages, so a package added later is guarded the day it arrives rather than the day someone extends a list; the walk skips `node_modules`, since a vendored dependency's lockfile is not this repo's to keep in sync. The derived result is itself pinned to an expected roster, because the failure mode of a derived set is that a mis-rooted or empty walk generates no assertions at all and reports a pass — re-rooting the walk drops the file from nine tests to three, every one of them green but for that pin. `site` is in scope despite being unpublished and parked at `0.1.0`: excluding it would carve a named exception into a guard whose whole point is that the roster is not curated, and its lockfile drifts like any other the first time the site is bumped. `reference-impl/examples/toy-repo` is out for a structural reason rather than an exception — it is a fixture with a `package.json` and no lockfile, so there is no pair to keep in sync. Each package's two fields are separate assertions, since one can be correct while the other is stale. The guard was green on arrival, all three pairs already in sync, so there was no pre-existing failure to drive it red-first and every assertion was proved by mutation instead — ten of them, each verified to have landed on disk before its red was believed, including a negative case planting a drifted lockfile under `node_modules` to show the skip is load-bearing. Not part of the published package.
- `tests/package-contract.test.ts` also guards the `VERSION` files, the same rule one file over and a worse one. `VERSION` is in the `files` array of both published packages, so unlike a lockfile it *ships* — a stale one is wrong in the installed tarball, not merely untidy in the repo. Of the six-site reference-impl bump list it was the entry carried by a release note alone: `package.json` is the source, `tests/skills.test.ts` trips on its own hardcoded assertion, and the lockfile pair is covered above. The `standard/VERSION` read already at the top of that file looks like it covered this and did not — it asks whether reference-impl's declared *range* (`^0.8.0`) admits the version, so `standard/VERSION` could sit at 0.8.2 against a `package.json` of 0.8.3 and satisfy it; mutating it to exactly that leaves both range assertions green, which is how the gap was confirmed rather than argued. The roster is two packages, not the three the lockfile walk finds: `site` has no `VERSION` file, being unpublished. Deriving it earns its keep in both directions here — adding a `VERSION` file to a new package trips the pin, and so does deleting one, which for `reference-impl` nothing else in the suite would have noticed. The walk that found lockfiles is generalised to take a filename and serves both rosters, so every lockfile assertion was re-proved against it rather than assumed unchanged. ⚠ `reference-impl/.claude-plugin/plugin.json` and `.claude-plugin/marketplace.json` carry versions too and are deliberately **not** in lockstep — they bump on feature releases only and the patch train leaves them behind, 0.13.2 against a `package.json` of 0.13.5 as this is written. Both were mutated to 9.9.9 to prove the guard stays green on them: they are decoys, not omissions. Sixteen mutations in total, each verified to have landed on disk. Not part of the published package.
