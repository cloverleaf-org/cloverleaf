...
7.3 **Reach the council verdict.**
   - **If `plan.aggregation === "chair"`:**
     ...
     - Else if **every member passed** (no member bounced **and** none escalated), the council verdict is `{"verdict":"pass","rule":"chair","rationale":"all members passed; chair not convened","members":[<members array>],"forward":[]}` — do **not** dispatch the chair.
