...
- `docs/extensions.md` and `schemas/extensions.schema.json` — both described the extension hatch as a Work Item mechanism while a shipped schema used it for something else. The guide opened *"All extensions live under the `extensions` field on a Work Item"* and the meta-schema's own `description` read *"Team-defined extension fields on Work Items"*, but `project.schema.json` — *"Per-project configuration"*, which does not reference `work-item.schema.json` and is not among the trackable units `work-item.schema.json` names — `$ref`s that same meta-schema exactly as `task` does. `docs/` is in this package's `files`, so the Standard contradicted itself in text that ships. Both statements are widened, and the boundary they were missing is now stated outright with its reason: descriptive documents (the four Work Item types, their abstract parent, and `project`) carry the hatch; behavioural configuration (`council-config`, `path-rules`, `risk-classifier-rules`) is deliberately closed, because `additionalProperties: false` on a file the engine reads is what turns a misspelled `counsil_profiles` into an error rather than a key that is silently ignored; protocol messages and generated artifacts are closed for the same reason. The guide also accounts for the places arbitrary data is accepted that are *not* the hatch, so its list is complete rather than merely correct: `problem` and `work-item` set `additionalProperties: true`, and `feedback`, closed at the envelope, gives every finding a `metadata` object whose shape the Standard deliberately does not constrain — the only nested open object it ships. **No schema changed.** The split was already correct — only the account of it was wrong, and the absence of the rationale was the whole defect.
...
- `conformance/tests/extension-boundary.test.ts` pins which schemas may carry the hatch, on two anchors that catch different failures. The set of hatched schemas is asserted against the ruling as a literal, so a seventh schema arriving on the wrong side of the boundary fails even if every document is updated to agree with it; and `docs/extensions.md`'s two boundary lists are checked against the schema directory, so the published text cannot drift back on its own. Either anchor alone has a blind spot the other covers — a derived-only check passes when both sides drift together, and a literal-only check passes while the prose goes stale, which is the failure being fixed here. Red-proved six ways: hatching a closed schema, unhatching a hatched one, dropping a schema from the doc's list, reverting the meta-schema description, rewording a section heading away, and — the case the two-anchor shape exists for — hatching `council-config` *and* moving it between the doc's lists so that both sides agree with each other, which the parity assertions accept and the literal pin still rejects. It also derives the set of nested open objects rather than spot-checking the known one, so the guide's claim that `feedback`'s finding `metadata` is the only one cannot quietly become false.
- The same suite records the count that every prior description of this boundary got wrong. The split is three-way, not two-way: `problem` and `work-item` set `additionalProperties: true` for reasons that are not the boundary rule — RFC 7807 §3.2 *mandates* prefixed extension members, and closing the abstract parent of RFC/Spike/Plan/Task would reject its own children — so the deliberately-closed set is **ten**, not eleven. Earlier counts measured the presence of an `extensions` property and never measured `additionalProperties`, which is the field that actually decides rejection.
